...
// To create a class definition, we use the `class` keyword followed
// by the name of the object and then curly braces:
class Empty {}

// Clases can have `properties` and they must have default values set.
// These defaults will be used when the object is created.
class Color {
    // Always include the type annotation. It's optionall, but
...
// We can create a new object using the `new` keyword. Creating
...
class Point {
    ...
    // allows us to easiy set values during creation.
    constructor(x: number, y: number) {
        this.x = x;
        this.y = y;
    }

    // We can create 'class emthods' wich can be called using dot notation.
